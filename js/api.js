...
const api = {
  ...
  async getHistory() {
    ...
    const user = DB.find('users', u => u.id === payload.id && !u.is_deleted);
  ...
  // Get ALL users' quiz history (for admin/view all)
  async getAllHistory() {
    const payload = this._verifyToken();
    if (!payload) throw new Error('Not authenticated');
    
    const sessions = DB.get('quiz_sessions');
    
    // Add user info and questions to each session
    const sessionsWithQuestions = sessions.map(session => {
      const user = DB.find('users', u => u.id === session.user_id && !u.is_deleted);
      const questions = DB.filter('quiz_questions', q => q.session_id === session.id);
      return {
        ...session,
